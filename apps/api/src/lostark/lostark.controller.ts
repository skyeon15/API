...
import { ApiTags, ApiOperation, ApiQuery, ApiResponse, ApiProduces, ApiBearerAuth, ApiUnauthorizedResponse, ApiForbiddenResponse, ApiHeader } from '@nestjs/swagger';
...
export class LostarkController {
  ...
  @ApiResponse({
    status: 200,
    description: '캐릭터 프로필 정보를 성공적으로 찾았어요. 정말 강력한 캐릭터네요!',
    content: {
      'application/json': {
        schema: {
          type: 'object',
      ...
      'text/plain': {
        example: "[ 에케의 로스트아크 ][br]조회시각 : 2026-03-26 07:31:06[br]Lv.60[br]서버 : 카제로스[br]PVP : 1급[br]칭호 : 파란대나무숲의 주인[br]길드 : 파란대나무숲[br]영지 : 에케영지 (Lv.70)[br][br]레벨 정보[br]원정대 : 300[br]전투 : 60[br]장착 아이템 : 1620.00[br]달성 아이템 : 1620.00"
      }
    }
  })
  @ApiResponse({
    status: 404,
    description: '아무리 찾아봐도 해당 캐릭터를 찾을 수 없어요. 닉네임이 정확한지 확인해 주세요!'
  })
  async getProfile(
    @Req() req: Request,
