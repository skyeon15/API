...
export class LostarkService {
  ...
  async getProfile(nickname: string): Promise<LostarkProfile> {
    ...
    try {
      ...
      const profileData = {
        name,
        server: $('.profile-character-info__server')
          .text()
          .replace('@', '')
          .trim(),
        level: $('.profile-character-info__lv')
          .text()
          .replace('Lv.', '')
          .trim(),
        lever_expedition: $('.level-info__expedition > span')
          .text()
          .replace('원정대 레벨Lv.', '')
          .trim(),
        level_fight: $('.level-info__item > span:nth-child(2)')
          .text()
          .replace('Lv.', '')
          .trim(),
        level_item: $('.level-info2__expedition > span:nth-child(2)')
          .text()
          .replace('Lv.', '')
          .trim(),
        level_item2: $('.level-info2__item > span:nth-child(2)')
          .text()
          .replace('Lv.', '')
          .trim(),
        ...
        wisdom_level: $('.game-info__wisdom > span:nth-child(2)')
          .text()
          .replace('Lv.', '')
          .trim(),
      };
      ...
      if (
        ...
        throw error;
      this.logger.error(
        `Error fetching Lost Ark profile for ${nickname}:`,
        error,
      );
      throw new InternalServerErrorException(
        '로스트아크 정보를 가져오는 중 오류가 발생했습니다.',
      );
    }
  }
