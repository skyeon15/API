...
export class NeisController {
  ...
  @ApiOperation({
    summary: '급식 메뉴',
    description:
      '오늘 점심이나 저녁에 무엇이 나오는지 궁금하신가요? 학교 이름을 입력해 주시면 메뉴를 알려드릴게요!',
  })
  @ApiParam({
    name: 'name',
    description:
      '메뉴가 궁금한 학교 이름이에요. (예: 서울고등학교, 부산대신중학교 등)',
    required: true,
    example: '서울고등학교',
  })
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        datetime: {
          type: 'string',
          description: '조회 기준 날짜 (YYYY-MM-DD hh:mm:ss)',
        },
        MEALS: {
          type: 'array',
          ...
          items: {
            ...
            properties: {
              meal: {
                type: 'string',
                description: '식사 구분 (예: 중식, 석식)',
              },
              dish: {
                type: 'string',
                description: '식단 요리명 (개행은 [br]로 표시됨)',
              },
            },
          },
        },
