...
export class AlimtalkController {
  ...
  @ApiOperation({
    summary: '카카오 카테고리 목록',
    description: '채널 등록 시 필요한 카카오 비즈니스 카테고리 목록이에요.',
  })
  getCategories() {
  ...
  @ApiOperation({
    summary: '채널 수정',
    description: '채널 별칭이나 활성 여부를 수정해요.',
  })
  ...
  getTemplates(
    ...
    return this.alimtalkService.getTemplates(
      this.ctx(apiKey, req).userId,
      channelId,
    );
  }
  ...
  @ApiOperation({
    summary: '템플릿 동기화',
    description: '최신 템플릿 정보를 DB에 동기화해요.',
  })
  @ApiBody({
    schema: {
      type: 'object',
      required: ['channelId'],
      properties: {
        channelId: { type: 'string', description: '동기화할 채널 ID' },
      },
    },
  })
  syncTemplates(
    ...
    return this.alimtalkService.syncTemplates(
      body.channelId,
      this.ctx(apiKey, req),
    );
  }
  ...
  @ApiOperation({
    summary: '템플릿 생성',
    description:
      '새 알림톡 템플릿을 생성하고 카카오에 등록해요. 생성 후 검수 요청이 필요해요.',
  })
  @ApiBody({
    schema: {
      type: 'object',
      ...
      properties: {
        ...
        title: {
          type: 'string',
          description: '강조표기형 전용 제목',
          nullable: true,
        },
        subtitle: {
          type: 'string',
          description: '강조표기형 전용 서브타이틀',
          nullable: true,
        },
        buttons: {
          type: 'array',
  ...
  @ApiOperation({
    summary: '템플릿 검수 요청',
    description: '카카오 비즈니스에 템플릿 검수를 요청해요.',
  })
  ...
  updateTemplate(
    ...
    return this.alimtalkService.updateTemplate(
      code,
      body,
      this.ctx(apiKey, req),
    );
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      '`db`: DB에서만 삭제, `kakao`: 카카오에서도 삭제 (기본값: `db`)',
  })
  deleteTemplate(
    @Param('code') code: string,
  ...
  @ApiOperation({
    summary: '알림톡 발송',
    description:
      '알림톡을 즉시 또는 예약 발송해요. `scheduledAt`이 없으면 즉시 발송이에요.',
  })
  @ApiResponse({
    status: 201,
    description:
      '발송 요청이 완료됐어요. 예약 발송인 경우 지정한 시각에 발송돼요.',
    type: GeneralResponseDto<SendResultDataDto>,
  })
  send(
  ...
  @ApiOperation({
    summary: '예약 발송 취소',
    description:
      '예약된 알림톡을 취소해요. 이미 발송된 메시지는 취소할 수 없어요.',
  })
  ...
  @ApiOperation({
    summary: '발송 결과 조회',
    description: '실시간으로 발송 결과를 조회하고 DB에 저장해요.',
  })
  @ApiParam({
    name: 'tid',
    description: '트랜잭션 ID',
